...
import { lowerAndResolve, hirChildren, type HirNode, type DefinitionId } from "@pglambda/hir";
 ...
 */
function findHirNodeAtPosition(root: HirNode, line: number, column: number): HirNode | null {
...
export function checkModule(
  ...
  if (!parseResult.success) {
    throw new Error(`Parse failed: ${parseResult.diagnostics.map((e) => e.message).join(", ")}`);
  }
